...
.controller('voteController', ['$scope', '$http', '$modal', function($scope, $http, $modal) {
	...
	$http.post('panel/controller.php?p=posterDetail', {pdfUrl:pdfUrl}).success(function(data){
		if (data['code'] && data['code'] == 'fail'){
			...
			if (projectN){
				$scope.modal = {
					title: ' Detail', 
					content: 'Project Name: <b>'+projectN+
						'</b><br/>Project Category: <b>'+category+
						'</b><br/>Supervisor: <b>'+supervisor+
						'</b><br/>Student 1: <b>'+studentN1+
						'</b><br/>Student 2: <b>'+studentN2+
						'</b><br/>Student 3: <b>'+studentN3+
						'</b><br/>email: <b><a target="_top" href="mailto:'+email+'">'+email+'</a>'+
						'</b><br/>'
				};
  ...
  $scope.addVote = function(poster){
	  var idDeadline = false;
	  //var idDeadline = true;
	  if (idDeadline){
		 confirm('Nomination has met the deadline, you can\'t vote!');
  ...
  $http.get('panel/controller.php?p=populateVotes' + graduateTypeVal, {}).success(function($data){
	//console.log($data);
	if ($data){
		if ($data.code == 'failed'){
			window.location.replace('/rc/panel/login.html');
	  ...
	  if ($data['vote4']){
		var imageId = $data['imageId4'];
		var imageName = $data['vote4'].split("/").pop().replace('_', ' ').replace('-', ' ');
		var imageUrl = $data['vote4'];
		var pdfUrl = $data['vote4'].replace('jpg', 'pdf');
		$scope.votes.push({imageId: imageId, imageName:imageName, imageUrl:imageUrl, pdfUrl:pdfUrl});
	  }
	  if ($data['vote5']){
		var imageId = $data['imageId5'];
		var imageName = $data['vote5'].split("/").pop().replace('_', ' ').replace('-', ' ');
		var imageUrl = $data['vote5'];
		var pdfUrl = $data['vote5'].replace('jpg', 'pdf');
		$scope.votes.push({imageId: imageId, imageName:imageName, imageUrl:imageUrl, pdfUrl:pdfUrl});
	  }
	}
  });
  ...
  $scope.submitVote = function() {
	var maxPosters = 3;//UndergraduatePM, PostgraduateCE, PostgraduatePM
	
	/*if (graduateTypeVal == 'UndergraduateCE'){
		maxPosters = 5;
	}*/
	
	if ($scope.votes.length > maxPosters){
	  alert('You have seleted more than '+maxPosters+' posters! Please remove some posters from your selection.');
	  return;
	}
	...
	//console.log($dataObj);
	$http.post('panel/controller.php?p=submitVote' + graduateTypeVal, $dataObj).success(function(data, status) {
		$scope.status = status;
		//$scope.data = data;
